...
export default function CharacterHistory({ history }: CharacterHistoryProps) {
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-amber-900/20 to-orange-900/20 border border-amber-500/30 rounded-2xl p-4 relative" // Reduced from p-6 to p-4 and border-2 to border
    >
      {/* Compact Header */}
      <div className="relative mb-4"> {/* Reduced from mb-6 to mb-4 */}
        <div className="flex items-center justify-center gap-2 mb-3"> {/* Reduced from gap-3 mb-4 to gap-2 mb-3 */}
          <div className="w-4 h-4 bg-amber-400 rounded-full"></div> {/* Reduced from w-6 h-6 to w-4 h-4 */}
          <h3 className="text-lg font-bold text-amber-400 tracking-wider">HALL OF SOULS</h3> {/* Reduced from text-xl to text-lg */}
      ...
      {history.length === 0 ? (
        <div className="text-center py-8"> {/* Reduced from py-12 to py-8 */}
          <motion.div
            animate={{ 
              scale: [1, 1.1, 1],
              rotate: [0, 5, -5, 0]
            ...
            className="text-4xl mb-3" // Reduced from text-6xl mb-4 to text-4xl mb-3
          >
            ⚱️
          </motion.div>
          <h4 className="text-amber-400 font-bold mb-2 text-sm">Empty Chronicle</h4> {/* Added text-sm */}
          <p className="text-amber-300/70 text-xs">No souls have been forged yet...</p> {/* Reduced from text-sm to text-xs */}
        </div>
      ) : (
        <div className="space-y-2 max-h-72 overflow-y-auto scrollbar-thin scrollbar-thumb-amber-500/30"> {/* Reduced from space-y-3 max-h-96 to space-y-2 max-h-72 */}
          <AnimatePresence>
            {history.map((char, index) => {
              ...
              return (
                <motion.div
                  ...
                  whileHover={{ scale: 1.01, x: 3 }} // Reduced from scale: 1.02, x: 5 to scale: 1.01, x: 3
                  transition={{ 
                    delay: index * 0.05, // Reduced from 0.1 to 0.05
                    type: "spring",
                    stiffness: 200
                  }}
                  className="bg-gradient-to-r from-amber-900/30 to-orange-900/30 border border-amber-500/30 rounded-lg p-3 hover:border-amber-400/60 transition-all duration-300 relative overflow-hidden" // Reduced from p-4 to p-3
                >
                  {/* Rarity Glow */}
                    ...
                    <div className="flex items-center justify-between mb-2"> {/* Reduced from mb-3 to mb-2 */}
                      <div className="flex items-center gap-2"> {/* Reduced from gap-3 to gap-2 */}
                        <div className="relative">
                          <div className="w-10 h-10 rounded-full overflow-hidden border border-amber-400/70 bg-gradient-to-br from-amber-900 to-orange-900"> {/* Reduced from w-12 h-12 border-2 to w-10 h-10 border */}
                            <Image
                      ...
                      <div className="text-right">
                        <div className="text-base font-bold text-amber-400">{totalStats}</div> {/* Reduced from text-lg to text-base */}
      ...
      <div className="mt-4 pt-3 border-t border-amber-500/30"> {/* Reduced from mt-6 pt-4 to mt-4 pt-3 */}
        <h4 className="text-xs font-bold text-amber-400 mb-2 tracking-wide">⚡ SOUL RARITY CODEX</h4> {/* Reduced from text-sm mb-3 to text-xs mb-2 */}
      ...
      <div className="mt-3 pt-3 border-t border-amber-500/30"> {/* Reduced from mt-4 pt-4 to mt-3 pt-3 */}
